...
#### 4-5-1. 銘柄カードの表示項目（カード表示時）

| 項目 | 内容 |
|------|------|
| 年間配当 | `現状の金額 / 目標の金額`（例: ¥12,000 / ¥18,000）。年間配当金カードの現状/目標トグル（税引前・税引後）の設定に準じて算出 |
| 株主優待 | `現状の金額 / 目標の金額`。優待設定がある銘柄のみ表示 |
| 配当/株（投資信託は分配金/万口） | 1株（1万口）あたりの配当・分配金額 |

> 目標株数（合計）が未設定（0）の銘柄は、目標側の金額を表示せず現状の金額のみ表示する。
> コンパクト表示では表示しない（カード表示のみ）。
